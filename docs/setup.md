...
5. 複製以 `.apps.googleusercontent.com` 結尾的 **Client ID**，在網站資料設定中儲存／準備授權，待準備完成再按連線。Client ID 是公開識別碼；不要填 Client secret。REST 呼叫使用 Bearer access token，本程式不需要 API key。
...
3. 換機時使用相同網站、Client ID 與 Google 帳號，查找此應用程式可存取的 Sheet，再載入原本那份。清單不是全部 Google Drive。若更換 Cloud 專案或複製／手動建立 Sheet，可能不在這個應用程式的檔案授權範圍內。

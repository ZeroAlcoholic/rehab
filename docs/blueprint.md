...
Android Chrome 上記錄、修改、沿用上次訓練、檢視趨勢；離線可保存，連線且 Google 授權有效時同步至私人 Google Sheet。換機可從同一份 Sheet 還原。GitHub Pages 僅部署靜態程式，不發布私人紀錄。
...
- 首頁：今日日期、同步狀態、記一筆、沿用上次、InBody、資料設定。
- 訓練表單：動作、機台識別、日期、lb/kg、動態組數（每組重量或輔助量與次數）、疼痛／動作狀況／備註。
- 沿用帶入上次內容並改成今日；新增與修改清楚區別。刪除需確認且以墓碑版本保留歷史。

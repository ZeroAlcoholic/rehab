...
- 新增、修改、刪除訓練；沿用最近一筆，或在歷史按「再練一次」。各組重量獨立保存，當次疼痛、動作狀況與備註重新記錄。
...
瀏覽器測試需指定 Python 環境的 Playwright 與已安裝 Microsoft Edge。它會啟動僅本機可達的臨時 HTTP server、獨立瀏覽器設定檔，結束後關閉。`artifacts/` 是測試產物，不發布；內含測試資料，正式備份請另存私人位置。
